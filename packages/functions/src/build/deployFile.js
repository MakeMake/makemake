...
requireAuth({}, ['https://www.googleapis.com/auth/cloud-platform']).then(
  async () => {
    try {
      ...
      const releases = response.body.releases

      releases.forEach((release) => {
        console.log(
          /* release.name, */ release.version.status,
          release.version.createTime,
          release.version.fileCount,
          release.version.name
        )
      })
      const latestVersion = releases[0].version.name

      // Get the files in the latest version
      console.log('Getting files in latest version...')
      response = await api.request('GET', `/v1beta1/${latestVersion}/files`, {
        auth: true,
      ...
      const zipstream = fs.createReadStream(process.cwd() + file).pipe(gzipper)
      ...
      console.log(response.body)
      const version = response.body.name
      ...
      const requiredHashes = response.body.uploadRequiredHashes
      const uploadUrl = response.body.uploadUrl

      if (requiredHashes && requiredHashes.includes(files[file])) {
            ...
            .pipe(
              request.post(reqOpts, function(err, res) {
                if (err) {
                  ...
                  return reject(
                    new Error('Unexpected error while uploading file.')
                  )
                }
                resolve()
              })
      ...
      if (!isDryRun) {
        ...
        response = await api.request(
          'PATCH',
          `/v1beta1/${version}?updateMask=status`,
          {
            origin: api.hostingApiOrigin,
            auth: true,
        ...
        response = await api.request(
          'POST',
          `/v1beta1/sites/${site}/releases?version_name=${version}`,
          {
            auth: true,
            origin: api.hostingApiOrigin,
        ...
        response = await api.request('DELETE', `/v1beta1/${version}`, {
          auth: true,
          origin: api.hostingApiOrigin
        })
        console.log(response.body)
